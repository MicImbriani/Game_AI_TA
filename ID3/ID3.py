...
def splitByAttribute(examples, attribute):
    ...
    for example in examples:
        ...
        if int(example[attribute_id]) in sets:
            temp = sets[example[attribute_id]]
            temp = np.vstack([temp, example])
        # Otherwise, create a new list.
        else:
            temp = example[np.newaxis]
        sets[example[attribute_id]] = temp
...
input = dataset[["Score", "Trend", "Win %", "Role %", "Pick %", "Ban %", "KDA"]].to_numpy()
